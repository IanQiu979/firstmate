...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, Zellij can authoritatively identify a missing recorded pane but otherwise remains unverified (it can never prove `dead`), and Orca and cmux do not support secondmate spawns.
