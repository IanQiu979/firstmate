...
# exact window; the Herdr adapter reuses its husk classifier. Zellij can
# authoritatively identify a missing recorded pane but otherwise remains
# unverified. Orca and cmux do not support secondmate spawns.
fm_backend_agent_state() {  # <backend> <target>
  ...
  case "$backend" in
    ...
    zellij) fm_backend_zellij_agent_state "$target" ;;
    *) printf 'unverified' ;;
  esac
}
