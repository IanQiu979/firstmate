...
config/harness-concurrency-limit  max LIVE agents fm-spawn.sh allows on one harness at a time within this home, counted across every project and task; LOCAL, gitignored; absent or non-positive-integer = default 3; not inherited, each home's cap is independent (docs/configuration.md "Per-harness concurrency cap")
...
7. **Context digest and next step** - last of the bulk sections, the full contents of `data/projects.md`, `data/secondmates.md`, `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, each clearly delimited, followed by an optional daily-briefing lint check and then the closing reminder.
   ...
   When `/Volumes/IanUSB` is mounted, `bin/fm-briefing-lint.sh` checks today's and yesterday's daily briefing files and reports violations as `BRIEFING_LINT:` lines; a missing volume or file is silent, clean files add no noise, and findings never gate session start.
